...
Same role as in Topics 1 and 2: SQLite is the raw-data store, used only for
basic CRUD on single tables. We read tables into pandas and do everything else
(feature building, model training) there.

This topic has a single raw table, ``prices``, plus a small ``provenance``
...
KNOWN_TABLES = {"prices", "provenance"}
