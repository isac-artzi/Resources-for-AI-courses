...
from src.model import split, train
...
@st.cache_resource
def get_trained_filter():
    """Load the corpus, split it, and train the filter once per app process.

    Returns the fitted pipeline plus the held-out test set, so the evaluate
    page can score exactly the model the classify page uses. ``cache_resource``
    keeps the trained model in memory instead of retraining on every click.
    """
    emails = load_table("emails")
    X_train, X_test, y_train, y_test = split(emails)
