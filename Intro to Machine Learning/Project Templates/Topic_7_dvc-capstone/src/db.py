...
CRUD on single tables. The Streamlit "Dataset" page reads the raw ``houses``
table from here; the reproducible DVC pipeline reads the same data from the raw
CSV. Everything heavier (cleaning, feature engineering, training) happens in
pandas + scikit-learn.

This topic has a single raw table, ``houses``, plus a small ``provenance`` table
...
KNOWN_TABLES = {"houses", "provenance"}
