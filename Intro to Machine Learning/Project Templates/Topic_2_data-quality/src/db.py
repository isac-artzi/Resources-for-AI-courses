...
Same role as in Topic 1: SQLite is the raw-data store, used only for basic CRUD
on single tables. We read tables into pandas and do everything else there.

This topic has a single raw table, ``signups``, plus a small ``provenance``
...
KNOWN_TABLES = {"signups", "provenance"}
