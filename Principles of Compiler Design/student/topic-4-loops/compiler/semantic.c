 ...
 *   • `break` is rejected outside a loop, using a loop-depth counter
 *
 * WHAT COMES NEXT
...
/* --------------------------------------------------------------
 * TODO (Topic 4)
 * A counter for how many loops (Topic 5: and switches) enclose the
 * statement being checked.  `break` is legal only when it is > 0.
 * -------------------------------------------------------------- */
...
/* --------------------------------------------------------------
 * TODO (Topic 4)
 * Check NODE_BREAK: it is legal only when breakDepth > 0, i.e. only
 * inside a loop (Topic 5 adds switch to that list).  This is the
 * smallest possible example of a CONTEXT-SENSITIVE rule — no grammar
 * can express it, which is exactly why semantic analysis exists.
 * -------------------------------------------------------------- */
