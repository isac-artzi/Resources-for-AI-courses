...
/* --------------------------------------------------------------
 * TODO (Topic 5)
 * Map the logical operator codes to TAC_AND, TAC_OR and TAC_NOT.
 * -------------------------------------------------------------- */
...
/* --------------------------------------------------------------
 * TODO (Topic 5)
 * Lower a NODE_IF to labels and jumps.  There is no `if` in TAC; the
 * shape you want is:
 *      t = <condition>
 *      IF_FALSE t GOTO Lelse
 *      <then branch>
 *      GOTO Lend
 *   Lelse:
 *      <else branch>
 *   Lend:
 * Handle the no-else case by jumping straight to Lend.
 * Every label you need comes from newLabel().
 * -------------------------------------------------------------- */
        case NODE_WHILE: {
            /* Generate labels */
...
/* --------------------------------------------------------------
 * TODO (Topic 5)
 * Lower NODE_SWITCH.  The shape that makes fall-through fall out for
 * free is a DISPATCH section followed by a BODY section:
 *
 *    DECL __swN ; __swN = <controlling expr>   (evaluated ONCE)
 *    t = __swN == c0 ; IF_FALSE t -> next ; GOTO Lbody0
 *    ... one test per case ...
 *    GOTO Ldefault (or Lend when there is no default)
 *  Lbody0: <body of case 0>      <- no jump at the end:
 *  Lbody1: <body of case 1>         control falls into the next body,
 *  Lend:                            which IS fall-through
 *
 * `break` becomes GOTO Lend, so push Lend on the break-label stack
 * before generating the bodies.
 * A jump TABLE is the classic alternative — faster, but only when the
 * case values are dense.  Discuss the trade-off in your write-up.
 * -------------------------------------------------------------- */
        case NODE_BREAK: {
