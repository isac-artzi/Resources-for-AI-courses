 ...
 *   • `break` is now also valid inside a switch
 *
 * WHAT COMES NEXT
...
/* --------------------------------------------------------------
 * TODO (Topic 5)
 * Check a NODE_IF: the condition is an expression, the branches are
 * statements.  Nothing about `if` needs a new kind of check — it just
 * needs all three children visited, which is the point: analysis
 * recurses over structure, and new structure means new recursion.
 * -------------------------------------------------------------- */
        case NODE_WHILE:
...
/* --------------------------------------------------------------
 * TODO (Topic 5)
 * Check a NODE_SWITCH.  Beyond visiting the children, a switch has
 * two rules of its own worth enforcing:
 *   • at most ONE default clause
 *   • no two case labels with the same constant value
 * Both are errors a student compiler can catch cheaply and a
 * programmer is genuinely glad to have caught.
 * Remember breakDepth++/-- around the clause bodies.
 * -------------------------------------------------------------- */
        case NODE_BREAK:
            if (breakDepth == 0) {
                fprintf(stderr, "\n╔════════════════════════════════════════════════════════════╗\n");
