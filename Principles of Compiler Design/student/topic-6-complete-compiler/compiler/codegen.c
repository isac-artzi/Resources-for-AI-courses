 ...
 *   • No new language features — this is the milestone where the compiler is measured, documented and stress-tested
 *
 * WHAT COMES NEXT
    ...
    for (TACInstr* c = tac->head; c; c = c->next) {
        ...
        for (TACInstr* i = c->next; i && i->op != TAC_FUNC_END; i = i->next) {
            ...
            case TAC_AND: case TAC_OR: {
                ...
                int d = defReg(i->result);
                fprintf(out, "    %-4s $t%d, $t%d, $t%d       # %s = %s %s %s\n",
                        i->op == TAC_AND ? "and" : "or", d, na, nb,
                        i->result, i->arg1, i->op == TAC_AND ? "&&" : "||", i->arg2);
                releaseReg(na); releaseReg(nb);
                break;
            }
            case TAC_NOT: {
                int a = operandReg(i->arg1);
                int d = defReg(i->result);
                fprintf(out, "    seq  $t%d, $t%d, $zero     # %s = !%s\n", d, a, i->result, i->arg1);
                break;
            }
