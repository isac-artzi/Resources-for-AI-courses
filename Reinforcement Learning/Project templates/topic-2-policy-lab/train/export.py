...
def export_tabular_policy(
    Q: np.ndarray,
    V: np.ndarray,
    policy: np.ndarray,
    path: str | pathlib.Path,
) -> dict[str, Any]:
    """A Q-table plus the value function and greedy policy it implies.

    Q alone is enough to SERVE — `api/policy.py` takes an argmax over a row and
    that is the whole forward pass. V and the greedy policy ride along because
    the "Value Map" tab has to render both value functions, and the alternative
    is a Streamlit process that recomputes them, which would put policy code in
    the presentation tier and break the rule this repository is organised
    around. Three extra arrays cost ~600 bytes.

    V is stored explicitly rather than recomputed as Q.max(axis=1) at load
    time: for the Monte Carlo artifact those two are DIFFERENT objects — one is
    a first-visit evaluation of the policy, the other the control agent's
    action-value maximum — and silently substituting one for the other in a
    comparison whose entire point is the gap between them would be exactly the
    wrong bug to have.
    """
    row = export_arrays({"Q": Q, "V": V, "pi": policy}, path)
    return row | {"kind": "tabular", "obs_dim": 1, "n_actions": int(np.shape(Q)[1])}
