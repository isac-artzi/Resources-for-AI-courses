...
def gradient_stats(experiment_id: str):
    """Per-update gradient statistics for one run — the headline chart's data.

    Note that the UI asks for ONE run at a time and stitches the arms together
    itself. A `/gradient_stats?arm=vpg%2Bbaseline` endpoint would be fewer round
    trips and would move the definition of "which runs belong to this arm" into
    the service tier, where it would be invisible to the reader of your report.
    Grouping in the tab, from `experiments.hyperparameters`, keeps the grouping
    rule where a reader can see it.
    """
    return call(f"/gradient_stats?experiment_id={experiment_id}", None, method="GET")
