...
class PolicyArtifactStore:
    ...
    def _load_one(self, path: pathlib.Path):
        ...
        # TOPIC 5: a policy-VALUE archive has two heads and is recognised by the
        # value head's key. Checked BEFORE the generic W0/b0 loop below, because
        # the generic loop would happily load `W0, b0, W1, b1` and hand back a
        # two-layer MLP with no policy head, no value head, and an output
        # dimension of 128 — which loads without error, serves without error,
        # and is completely wrong. A loader that silently accepts an archive it
        # does not understand is worse than one that refuses it.
        if "Wv" in keys and "Wp" in keys:
            from search.net import PolicyValueNet

            pol = PolicyValueNet.from_npz(path)
            meta |= {
                "kind": "value-net",
                "obs_dim": pol.obs_dim,
