...
class Settings(BaseSettings):
    ...
    # Which artifact `policy_name="default"` resolves to. This product ships
    # FOUR `.npz` files — three agents plus the base template's smoke-test
    # table — so "the only one" is no longer a rule that can pick. The default
    # below is the name train/train.py writes for the A2C agent, which means a
    # fresh fork resolves correctly with no .env at all. Point it elsewhere when
    # you promote a different agent to be the one the Play tab opens on.
    default_policy: str = Field(default="a2c_cartpole", alias="DEFAULT_POLICY")
