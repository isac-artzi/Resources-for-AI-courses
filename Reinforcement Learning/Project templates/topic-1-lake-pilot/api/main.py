...
def rollout(req: RolloutRequest) -> RolloutResponse:
    ...
    for ep in range(req.episodes):
        ...
        for step in range(req.max_steps):
            ...
            # `req.deterministic` rather than a hard-coded True: the "Watch"
            # tab contrasts the trained agent (greedy) with an untrained one
            # (sampled from an all-zero table, i.e. uniform). Both go through
            # this one loop, so the comparison the stakeholder sees is produced
            # by the same code path that produces the number in the report.
            action, _ = policy.act(state_vec, deterministic=req.deterministic)
    ...
    # The syllabus asks audit_log to carry every /act AND every /rollout call.
    # One row per call, not one per step: the trajectory is already in the
    # response and 20 episodes x 200 steps of audit rows would be 4,000 inserts
    # for one button press on the free tier.
    try:
        get_store().log_audit(
            {
                "endpoint": "/rollout",
                "policy_sha256": meta["sha256"],
                "state_hash": f"episodes={req.episodes},seed={req.seed}",
                "action": f"greedy={req.deterministic}",
            }
        )
    except Exception:
        pass  # telemetry must never take down the request path
...
def episodes(experiment_id: str, limit: int = 20_000) -> EpisodesResponse:
    """The per-episode telemetry for ONE experiment — the Compare tab's data source.

    This endpoint exists so that the presentation tier never issues its own
    query for the learning curves. The rule in this repository is that the UI
    reads through a contract; adding a tab must not mean adding SQL to
    ui/app.py.

    `limit` is capped, and the response says when it truncated. Twenty thousand
    episodes serialise to roughly a megabyte of JSON, which the free tier will
    serve but which a browser tab left open on a slider will request over and
    over. A curve that silently stops early is worse than a slow one: it looks
    like an agent that stopped learning.
    """
