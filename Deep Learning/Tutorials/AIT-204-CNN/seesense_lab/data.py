...
def build_train_transform(augment: bool = True) -> transforms.Compose:
    ...
    # TODO 2.1: build a transforms.Compose.
    #   If `augment`: RandomCrop(32, padding=4) then RandomHorizontalFlip().
    #   Always finish with ToTensor() and Normalize(CIFAR10_MEAN, CIFAR10_STD).
    #   Order matters: crop and flip act on PIL images, so they come BEFORE ToTensor.
