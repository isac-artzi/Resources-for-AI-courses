...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # TODO 3.1: add the skip path to the body's output, THEN apply ReLU.
        #   y = ReLU(shortcut(x) + body(x))
        #   Check yourself: if body(x) is all zeros and the shortcut is the identity,
        #   what should y be? (Topic 3 Exercises, question 10.)
